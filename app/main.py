...
app = FastAPI(title="AI Trading Analyse", version="1.0.0")

# Mount /static only if the directory exists. On Vercel (read-only, serverless)
# a missing static dir would raise at import time and crash the function.
...
def home(request: Request):
    """Home → the live trading dashboard (crypto + stocks)."""
    return templates.TemplateResponse(request, "dashboard.html", {})


@app.get("/signals", response_class=HTMLResponse)
def signals_page(request: Request, filter: Optional[SignalType] = None):
    """The manual signals list page (kept for reference)."""
